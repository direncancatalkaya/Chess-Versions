...
namespace Chess
{
    ...
    public abstract class Tas
    {
        ...
        public virtual void RookMove(int x, int y)  // Taşı Hedef Konuma Götürür ve Buttonun çizim işlemlerini yapar ..
        {
            //FillAllCanGoList();
            //FillAttackList();
            //if (!FakeMove(x, y))
            ...
            //if (TasTipi == TasTipi.Sah)
            //{
            //    foreach (Kordinat VARIABLE in this.KordinatsCanGo)
            //    {
            //        if (VARIABLE.X == x && VARIABLE.Y == y && VARIABLE.KordinatType == KordinatType.BlackLongRook)
            //        {

            //            Move(1, 7);
            //            Form1.Squares[7, 0].Tas.Move(3, 7);
            //            break;

            //        }
            //        else if (VARIABLE.X == x && VARIABLE.Y == y && VARIABLE.KordinatType == KordinatType.BlackShortRook)
            //        {

            //            Move(5, 7);
            //            Form1.Squares[7, 7].Tas.Move(3, 7);
            //            return;
            //        }
            //        else if (VARIABLE.X == x && VARIABLE.Y == y && VARIABLE.KordinatType == KordinatType.WhiteLongRook)
            //        {

            //            Move(1, 0);
            //            Form1.Squares[0, 0].Tas.Move(3, 0);
            //            return;
            //        }
            //        else if (VARIABLE.X == x && VARIABLE.Y == y && VARIABLE.KordinatType == KordinatType.WhiteShortRook)
            //        {

            //            Move(5, 0);
            //            Form1.Squares[0, 7].Tas.Move(5, 0);
            //            return;
            //        }
            //    }
        ...
        public virtual void Move(int x, int y)  // Taşı Hedef Konuma Götürür ve Buttonun çizim işlemlerini yapar ..
        {
            ...
            if (TasTipi == TasTipi.Sah)
            {
                foreach (Kordinat VARIABLE in this.KordinatsCanGo)
                {
                    if (VARIABLE.X == x && VARIABLE.Y == y)
                    {
                        if (VARIABLE.KordinatType == KordinatType.BlackLongRook)
                        {
                            RookMove(1, 7);
                            Form1.Squares[7, 0].Tas.RookMove(3, 7);
                            return;

                        }

                        else if (VARIABLE.KordinatType == KordinatType.BlackShortRook)
                        {
                            RookMove(6, 7);
                            Form1.Squares[7, 7].Tas.RookMove(5, 7);
                            return;
                        }

                        else if (VARIABLE.KordinatType == KordinatType.WhiteLongRook)
                        {
                            RookMove(1, 0);
                            Form1.Squares[0, 0].Tas.RookMove(3, 0);
                            return;
                        }
                        else if (VARIABLE.KordinatType == KordinatType.WhiteShortRook)
                        {
                            RookMove(6, 0);
                            Form1.Squares[0, 7].Tas.RookMove(5, 0);
                            return;
                        }
            ...
            foreach (Kordinat VARIABLE in this.KordinatsCanGo)
            {
                if (VARIABLE.X == x && VARIABLE.Y == y && (VARIABLE.KordinatType==KordinatType.Attack || VARIABLE.KordinatType==KordinatType.Normal))
                {
